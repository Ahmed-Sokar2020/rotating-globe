...
export default function Globe2 () {
    ...
    function initGlobe () {
        ...
        setTimeout(() => {
            ...
            myGlobe.labelText("city").addEventListener('mouseover',handleCityHover)
        },1000)
    ...
    function handleCityHover(event:any) {
        const hoveredCity = event.target;
        const hoveredCityData = map.maps.find(city => city.city === hoveredCity.text);

        if (hoveredCityData) {
            const countryName = hoveredCityData.country;
            setHoveredCity(countryName)
        }
    }
